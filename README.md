...
This integration uses the same public tracking endpoint as the Slovak Parcel Service consumer website. Its fixed website transport key can be rotated by the carrier; an authorization failure stops updates and must be reported to the maintainers. It is not affiliated with, endorsed by, or supported by Slovak Parcel Service.
