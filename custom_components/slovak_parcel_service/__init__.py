"""Slovak Parcel Service parcel tracker custom component for Home Assistant."""
...
from .api import SlovakParcelServiceApiClient
...
@dataclass
class SlovakParcelServiceData:
    """Runtime data attached to the Slovak Parcel Service config entry."""

    client: SlovakParcelServiceApiClient
    coordinator: SlovakParcelServiceCoordinator


type SlovakParcelServiceConfigEntry = ConfigEntry[SlovakParcelServiceData]


async def async_setup_entry(hass: HomeAssistant, entry: SlovakParcelServiceConfigEntry) -> bool:
    """Set up Slovak Parcel Service from a config entry."""
    # No auth: Slovak Parcel Service tracking is public, so the HA-managed session is fine.
    client = SlovakParcelServiceApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = SlovakParcelServiceData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: SlovakParcelServiceConfigEntry) -> bool:
    """Unload the Slovak Parcel Service config entry."""
