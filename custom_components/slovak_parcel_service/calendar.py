"""Calendar platform for the Slovak Parcel Service parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Slovak Parcel Service deliveries calendar from a config entry."""
    async_add_entities([SlovakParcelServiceDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class SlovakParcelServiceDeliveriesCalendar(CoordinatorEntity[SlovakParcelServiceCoordinator], CalendarEntity):
    """A read-only calendar of expected Slovak Parcel Service deliveries.
    ...
    def __init__(self, coordinator: SlovakParcelServiceCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Slovak Parcel Service parcel")
            description_parts = [
