"""Constants for the Slovak Parcel Service parcel tracker integration."""
from enum import StrEnum

from homeassistant.const import Platform

DOMAIN = "slovak_parcel_service"
...
# ``INFO_API_URL`` is a second endpoint on the same host that fills in
# sender/weight (and, only with a matching ``recipientzip`` header, receiver)
# — see api.py's ``async_get_shipment_info``; ``TRACKING_URL`` is the
# human-facing deep link surfaced on each parcel's ``url`` field.
TRACKING_API_URL = "https://rest.mysps.sk/tracking/getShipmentTrackAndTrace"
INFO_API_URL = "https://rest.mysps.sk/tracking/getShipmentInfo"
TRACKING_URL = "https://mysps.sk/sk/tracking/{tracking_code}"

# This value is the public MySPS web application's fixed transport key.  It is
# a documented, per-carrier maintainer exception; it is never user input and
# must not appear in diagnostics, fixtures, logs or documentation.
API_KEY = "6VS314WZ+YtFPrYqqV2EZAoxKJNIyr13JGKTWjlOy6jz5zWKr-C8ZnpCJ7ChvZfW"
...
# Delivery postcode, asked once at setup (hub-level) and used as the
# ``recipientzip`` header on the shipment-info call so the receiver can be
# authorised. One hub per postcode: this is the hub's identity key
# (config_flow.py sets it as the unique_id, with no country prefix — this
# carrier only ever serves Slovakia, so a postcode is already unambiguous on
# its own). Fixed at setup, not editable afterwards: an identity key must not
# change under a configured entry, so getting a different postcode means
# adding a new hub, not editing this one.
