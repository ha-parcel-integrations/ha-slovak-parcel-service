"""Coordinator for the Slovak Parcel Service parcel tracker integration.
...
from .api import SlovakParcelServiceApiClient, SlovakParcelServiceApiError
from .const import (
...
class SlovakParcelServiceCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    def __init__(
        self,
        hass: HomeAssistant,
        client: SlovakParcelServiceApiClient,
        entry: ConfigEntry,
    ) -> None:
        ...
        # tracking_code -> extract_shipment_info() result. Sender/weight/
        # receiver are static shipment attributes that never change once
        # known, so a parcel's shipment-info call is fetched at most once per
        # its whole lifetime (not once per poll) — this is what keeps the
        # second HTTP call from doubling the coordinator's steady-state
        # request rate. A code that fails the call stays uncached and is
        # retried on the next refresh. Lives for the integration's lifetime
        # (resets on restart).
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        self._info_cache = {
            code: info for code, info in self._info_cache.items() if code in tracked_codes
        }
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (SlovakParcelServiceApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, SlovakParcelServiceApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("Slovak Parcel Service fetch failed for %s: %s", code, result)
        ...
        # Shipment-info enrichment (sender/weight/receiver): only for codes
        # whose tracking payload is genuinely fresh this cycle and not
        # already cached — see the ``_info_cache`` note in __init__ for why
        # one successful fetch covers a parcel's whole lifetime. A failure
        # here is never fatal to the parcel: it's logged and left uncached
        # for a retry next cycle, and sender/weight/receiver simply stay
        # ``None`` this time — the tracking call above remains the primary
        # source. A 429 still feeds the same backoff as the tracking call,
        # since both endpoints share the one rate limit.
        codes_needing_info = [code for code in fresh_codes if code not in self._info_cache]
        if codes_needing_info:
            info_results = await asyncio.gather(
                *(
                    self._client.async_get_shipment_info(
                        code, recipient_zip=postal_code
                    )
                    for code in codes_needing_info
                ),
                return_exceptions=True,
            )
            for code, result in zip(codes_needing_info, info_results):
                if isinstance(result, BaseException):
                    if not isinstance(
                        result, (SlovakParcelServiceApiError, aiohttp.ClientError)
                    ):
                    ...
                    if (
                        ...
                        if result.retry_after is not None:
                            retry_afters.append(result.retry_after)
                    _LOGGER.warning(
                        "Slovak Parcel Service shipment-info fetch failed for %s: %s",
                        code,
                        result,
                    )
                    continue
                self._info_cache[code] = extract_shipment_info(result)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "Slovak Parcel Service rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("Slovak Parcel Service unreachable for all tracked parcels")
